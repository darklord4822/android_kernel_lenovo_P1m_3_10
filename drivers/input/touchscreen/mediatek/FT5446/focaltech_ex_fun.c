...
u8 origin_chip_id_value = 0;

struct i2c_client *G_Client;
static unsigned char CTPM_FW_SHENYUE_BLACK[] = {
#include "ft5446i_shenyue_black.i"
};

static unsigned char CTPM_FW_SHENYUE_WHITE[] = {
#include "ft5446i_shenyue_white.i"
};


static unsigned char CTPM_FW_OFILM_BLACK[] = {
#include "ft5446i_ofilm_black.i"
};

static unsigned char CTPM_FW_OFILM_WHITE[] = {
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	if(origin_chip_id_value == 0x02){//shenyue black
		fw_len = sizeof(CTPM_FW_SHENYUE_BLACK);
	}else if(origin_chip_id_value == 0xfd){//shenyue white
		fw_len = sizeof(CTPM_FW_SHENYUE_WHITE);
	}else if(origin_chip_id_value == 0x51){//ofilm white
		fw_len = sizeof(CTPM_FW_OFILM_WHITE);
	}else if(origin_chip_id_value == 0xAE){//ofilm black
		fw_len = sizeof(CTPM_FW_OFILM_BLACK);
	}else{
		fw_len = 0;
	}
	 ...
	 */
	 FT_CTP_DMESG("IC TYPE IS :0x%x\n",fts_updateinfo_curr.CHIP_ID);
	 if (fts_updateinfo_curr.CHIP_ID==0x54)
	{
		if (fw_len < 8 || fw_len > 54 * 1024) 
		{
			FT_CTP_DEBUG("%s:FW length error\n", __func__);
			return -EIO;
		}

		if(origin_chip_id_value == 0x02){//shenyue black
			pbt_buf = CTPM_FW_SHENYUE_BLACK;
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_SHENYUE_BLACK));
		}else if(origin_chip_id_value == 0xfd){//shenyue white
			pbt_buf = CTPM_FW_SHENYUE_WHITE;
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_SHENYUE_WHITE));
		}else if(origin_chip_id_value == 0x51){//ofilm white
			pbt_buf = CTPM_FW_OFILM_WHITE;
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_OFILM_WHITE));
		}else if(origin_chip_id_value == 0xae){//ofilm black
			pbt_buf = CTPM_FW_OFILM_BLACK;
			i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_OFILM_BLACK));
		}else{
			FT_CTP_ERROR("module id not found:%x", origin_chip_id_value);
		}
		
		if (i_ret != 0)
			FT_CTP_DEBUG("%s:upgrade failed. err.\n",__func__);
	}	
...
{
	u16 ui_sz;
	u8 file_fw_version = 0;
	
	if(origin_chip_id_value == 0x02){//shenyue black
		ui_sz = sizeof(CTPM_FW_SHENYUE_BLACK);
		if (ui_sz > 2)
		{
			if(fts_updateinfo_curr.CHIP_ID==0x36)
				file_fw_version = CTPM_FW_SHENYUE_BLACK[0x10a];
			else if(fts_updateinfo_curr.CHIP_ID==0x58)
				file_fw_version = CTPM_FW_SHENYUE_BLACK[0x1D0A];//0x010A + 0x1C00
			else
				return CTPM_FW_SHENYUE_BLACK[ui_sz - 2];
		}

	}else if(origin_chip_id_value == 0x51){//ofilm white
		ui_sz = sizeof(CTPM_FW_OFILM_WHITE);
		if (ui_sz > 2)
		{
			if(fts_updateinfo_curr.CHIP_ID==0x36)
				file_fw_version = CTPM_FW_OFILM_WHITE[0x10a];
			else if(fts_updateinfo_curr.CHIP_ID==0x58)
				file_fw_version = CTPM_FW_OFILM_WHITE[0x1D0A];//0x010A + 0x1C00
			else
				return CTPM_FW_OFILM_WHITE[ui_sz - 2];
		}
		
	}else if(origin_chip_id_value == 0xAE){//ofilm black
		ui_sz = sizeof(CTPM_FW_OFILM_BLACK);
		if (ui_sz > 2)
		{
			if(fts_updateinfo_curr.CHIP_ID==0x36)
				file_fw_version = CTPM_FW_OFILM_BLACK[0x10a];
			else if(fts_updateinfo_curr.CHIP_ID==0x58)
				file_fw_version = CTPM_FW_OFILM_BLACK[0x1D0A];//0x010A + 0x1C00
			else
				return CTPM_FW_OFILM_BLACK[ui_sz - 2];
		}
		
	}else if(origin_chip_id_value == 0xFD){//shenyue white
		ui_sz = sizeof(CTPM_FW_SHENYUE_WHITE);
		if (ui_sz > 2)
		{
			if(fts_updateinfo_curr.CHIP_ID==0x36)
				file_fw_version = CTPM_FW_SHENYUE_WHITE[0x10a];
			else if(fts_updateinfo_curr.CHIP_ID==0x58)
				file_fw_version = CTPM_FW_SHENYUE_WHITE[0x1D0A];//0x010A + 0x1C00
			else
				return CTPM_FW_SHENYUE_WHITE[ui_sz - 2];
		}
		
	}else{//other
		file_fw_version = 0xFF;
	}

	return file_fw_version;	
}
...
static unsigned char ft5x46_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		/*********Step 2:Enter upgrade mode *****/
		FT_CTP_DEBUG("[FTS] Step 2:Enter upgrade mode \n");
		#if 0
		...
		FT_CTP_DEBUG("[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			&& reg_val[1] != 0) {

			FT_CTP_DEBUG("[FTS] Step 3 ok: CTPM ID,ID1 = 0x%x,ID2 = 0x%x ,0x%x, 0x%x:\n",
	...
	{
		FT_CTP_DEBUG("[FTS] FTS_UPGRADE_LOOP is  i = %d \n", i);
		return -EIO;
	}
	FT_CTP_DEBUG("[FTS] OK: FTS_UPGRADE_LOOP is  i = %d \n", i);
	auc_i2c_write_buf[0] = 0xcd;
	fts_i2c_Read(client, auc_i2c_write_buf, 1, reg_val, 1);
	FT_CTP_DEBUG("[FTS]bootloader version = 0x%x\n", reg_val[0]);
	...
	for(i = 0;i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (i_ret < 0)
		{
			FT_CTP_DEBUG( "[FTS] Step 4: read vendor id from flash error when i2c write, i_ret = %d\n", i_ret);
			continue;
		}
		//fts_i2c_Read(client, auc_i2c_write_buf, 4, reg_val, 2);
		...
		FT_CTP_DEBUG("%s: REG VAL ID1 = 0x%x,ID2 = 0x%x\n", __func__,reg_val[0],reg_val[1]);	
		break;
		
		
	}
	FT_CTP_DEBUG("Step 7: reset the new FW\n");
	auc_i2c_write_buf[0] = 0x07;
	fts_i2c_Write(client, auc_i2c_write_buf, 1);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	u8 uc_tp_fm_ver, uc_reg_addr, uc_reg_value;
	
	int i_ret;
	int is_ic_fw_crashed = 0;

	//step 1 : read vendor_id & fw_version
	uc_reg_addr = FTS_REG_VENDOR_ID;
	fts_i2c_Write(client, &uc_reg_addr, 1);
	fts_i2c_Read(client, &uc_reg_addr, 0, &uc_reg_value, 1);
	FT_CTP_DMESG("tp vendor id = 0x%x\n", uc_reg_value);
	
	i_ret = fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
	FT_CTP_DMESG("[upgrade],ic_fw_version:0x%x", uc_tp_fm_ver);

	//step 2 : check if ic firmware is crashed
	//if vendor_id is not shenyue or ofilm or fw_version is upper than 0x20, then firmware is wrong, need upgrade
	//ofilm, white , id=0x51
	//ofilm, black, id=0xAE
	//shenyue, white, id=oxfd
	//shenyue, black, id=0x02
	if(((uc_reg_value != 0x02)&&(uc_reg_value != 0x51)&&(uc_reg_value != 0xFD)&&(uc_reg_value != 0xAE))||(uc_tp_fm_ver > 0x20)){
		origin_chip_id_value = ft5x46_ctpm_VidFWid_get_from_boot(client);
		FT_CTP_DEBUG("force read id:0x%x", origin_chip_id_value);
		if((origin_chip_id_value == 0x02) || (origin_chip_id_value == 0x51)  || (origin_chip_id_value == 0xFD)  || (origin_chip_id_value == 0xAE)){
			is_ic_fw_crashed = 1;
		}else{
			return  -EIO;
		}
	}else{//firmware is right
		origin_chip_id_value = uc_reg_value;
	}

	
	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
	FT_CTP_DEBUG("[upgrade],file_fw_version:0x%x", uc_host_fm_ver);

	if ((uc_tp_fm_ver < uc_host_fm_ver)||(is_ic_fw_crashed == 1)) 
	{
		msleep(100);
		FT_CTP_DMESG("[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
		i_ret = fts_ctpm_fw_upgrade_with_i_file(client);
		if (i_ret == 0)	
		{
			msleep(300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			FT_CTP_DMESG("[FTS] upgrade to new version 0x%x\n",uc_host_fm_ver);
		} 
		else
		{
...
//	int i_ret;
	...
	if(pbt_buf[0] != 0x02)
	{
		FT_CTP_DEBUG("[FTS] FW first byte is not 0x02. so it is invalid \n");
		return -1;
	}
	...
	if(dw_lenth > 0x11f)
	{
		...
		{
			FT_CTP_DEBUG("[FTS] Fw length is invalid \n");
			return -1;
		}
	}
	else
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		{
			FT_CTP_DEBUG("[FTS] Step 3: GET CTPM ID OK,ID1 = 0x%x,ID2 = 0x%x\n",
					reg_val[0], reg_val[1]);
			break;
		} 
		else 
	...
	FT_CTP_DEBUG("Step 4:erase app and panel paramenter area\n");
	...
	for(i = 0;i < 200;i++)
	{
		...
		{
			FT_CTP_DEBUG("[FTS] erase app finished \n");
			break;
		}
		msleep(50);
	}
	...
	bt_ecc = 0;
	FT_CTP_DEBUG("Step 5:write firmware(FW) to ctpm flash\n");
	...
	for (j = 0; j < packet_number; j++) 
	{
		...
		for(i = 0;i < 30;i++)
		{
			...
			{
				FT_CTP_DEBUG("[FTS] write a block data finished \n");
				break;
			}
			msleep(1);
		}
	...
	FT_CTP_DEBUG("Step 6: read out checksum\n");
...
int fts_6x06_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
				&& reg_val[1] == fts_updateinfo_curr.upgrade_id_2) 
		{
			FT_CTP_DEBUG("[FTS] Step 3: CTPM ID OK ,ID1 = 0x%x,ID2 = 0x%x\n",
					reg_val[0], reg_val[1]);
			break;
		} 
		else 
...
//	int x=0;
	...
	if (i_ret == 0) 
	{
		FT_CTP_DEBUG("HidI2c change to StdI2c fail ! \n");
	}
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (i_ret < 0) 
		{
			FT_CTP_DEBUG("failed writing  0x55 and 0xaa ! \n");
			continue;
		}
		...
		if(i_ret < 0)
		{
			FT_CTP_DEBUG("failed to Switch HidtoI2c Protocol ! \n");
			continue;
		}
		...
		{	
			...
			FT_CTP_DEBUG("[FTS] Step 3: READ OK CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n", reg_val[0], reg_val[1]);
			break;
		} 
		else 
		{
	...
	reg_val[0] = reg_val[1] = 0x00;
	fts_i2c_Read(client, auc_i2c_write_buf, 1, reg_val, 1);
	FT_CTP_DEBUG("Checksum FT5X26:%X %X \n", reg_val[0], bt_ecc);
	if (reg_val[0] != bt_ecc) 
	{
		dev_err(&client->dev, "[FTS]--ecc error! FW=%02x bt_ecc=%02x\n",reg_val[0],bt_ecc);
		return -EIO;
	...
	FT_CTP_DEBUG("Step 8: Disable Write Flash\n");
    	auc_i2c_write_buf[0] = 0x04;
    	fts_i2c_Write(client, auc_i2c_write_buf, 1);
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	u8  	bt_ecc;
	int	i_ret;
	int	fw_filenth = sizeof(CTPM_FW_SHENYUE_BLACK);

	if(CTPM_FW_SHENYUE_BLACK[fw_filenth-12] == 30)
	{
		is_5336_fwsize_30 = 1;
	}
	else 
...
int fts_5x22_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	FT_CTP_DMESG("[FTS][%s] erase app area reg_val[0] = %x reg_val[1] = %x \n", __func__, reg_val[0], reg_val[1]);
	...
	for (j = 0; j < packet_number; j++) 
	{
		...
		FT_CTP_DMESG("[FTS][%s] bt_ecc = %x \n", __func__, bt_ecc);
		if (bt_ecc != bt_ecc_check)
			FT_CTP_ERROR("[FTS][%s] Host checksum error bt_ecc_check = %x \n", __func__, bt_ecc_check);
		fts_i2c_Write(client, packet_buf, FTS_PACKET_LENGTH + 6);
		/*msleep(10);*/
		for (i = 0; i < 30; i++) 
		{
			...
			FT_CTP_DMESG("[FTS][%s] reg_val[0] = %x reg_val[1] = %x \n", __func__, reg_val[0], reg_val[1]);
			msleep(1);
		}
	}
	if ((dw_lenth) % FTS_PACKET_LENGTH > 0) 
	{
		...
		FT_CTP_DMESG("[FTS][%s] bt_ecc = %x \n", __func__, bt_ecc);
		if (bt_ecc != bt_ecc_check)
			FT_CTP_DMESG("[FTS][%s] Host checksum error bt_ecc_check = %x \n", __func__, bt_ecc_check);
		for (i = 0; i < 30; i++) 
		{
			auc_i2c_write_buf[0] = 0x6a;
			reg_val[0] = reg_val[1] = 0x00;
...
int  fts_5x46_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	if(i_ret == 0)
	{
		FT_CTP_DEBUG("[FTS] hid change to i2c fail ! \n");
	}
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if(i_ret < 0)
		{
			FT_CTP_DEBUG("[FTS] failed writing  0x55 and 0xaa ! \n");
			continue;
		}
