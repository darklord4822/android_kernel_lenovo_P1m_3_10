...
static int fts_rw_iic_drv_myread(struct i2c_client *client, u8 *buf, int length)
{
	...
	if(ret<0)
	{
		FT_CTP_ERROR("%s:IIC Read failed\n",__func__);
	}
	return ret;
}
/************************************************************************
...
static int fts_rw_iic_drv_mywrite(struct i2c_client *client, u8 *buf, int length)
{
	...
	if(ret<0)
	{
		FT_CTP_ERROR("%s:IIC Write failed\n",__func__);
	}
	return ret;
}
/************************************************************************
...
static void fts_rw_iic_drv_setup_cdev(struct fts_rw_i2c_dev *dev, int index)
{
	...
	if (err)
		FT_CTP_ERROR("Error %d adding LED%d", err, index);
}
/************************************************************************
...
static int fts_rw_iic_drv_myinitdev(struct i2c_client *client)
{
	...
	if (err < 0) 
	{
		FT_CTP_ERROR( "%s:ft_rw_iic_drv failed  error code=%d---\n",
				__func__, err);
		return err;
	}
	...
	if (!fts_rw_i2c_dev_tt)
	{
		...
		FT_CTP_ERROR("%s:ft_rw_iic_drv failed\n",
				__func__);
		return err;
	}
	...
	if (IS_ERR(fts_class)) 
	{
		FT_CTP_ERROR("%s:failed in creating class.\n",
				__func__);
		return -1; 
	} 
	/*create device node*/
