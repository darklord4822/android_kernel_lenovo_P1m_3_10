...
/*Used for TP Test */                      
#define CTP_OPEN_SHORT_TEST                                                                                      
//#define TP_TESTRESULTPROC                                    
...
#ifdef CONFIG_TOUCHSCREEN_FT5446_GESTURE
#define WT_CTP_GESTURE_SUPPORT	1
#else
#define WT_CTP_GESTURE_SUPPORT	0
#endif
...
#define FT_CTP_TAG "FOCAL-TP-TAG"
#define FT_CTP_DMESG(fmt,arg...)          xlog_printk(ANDROID_LOG_INFO,FT_CTP_TAG,"<<-CTP-INFO->> "fmt,##arg)
#define FT_CTP_ERROR(fmt,arg...)          xlog_printk(ANDROID_LOG_INFO,FT_CTP_TAG,"<<-CTP-ERROR->> "fmt,##arg)
#define FT_CTP_DEBUG(fmt,arg...)          do{\
                                            if(CTP_DEBUG_ON)\
                                            xlog_printk(ANDROID_LOG_INFO,FT_CTP_TAG,"<<-CTP-DEBUG->> [%d]"fmt,__LINE__, ##arg);\
                                        }while(0)
