...
/*----------------------------------------------------------------------------*/
static int kxtj2_1009_remap_to_gravity(struct i2c_client *client, int data[KXTJ2_1009_AXES_NUM]);
...
/*----------------------------------------------------------------------------*/
static int KXTJ2_1009_WriteCalibration_New(struct i2c_client *client, struct GS_CALI_DATA_STRUCT *data_cali)
{
	struct kxtj2_1009_i2c_data *obj = i2c_get_clientdata(client);
	GSE_FUN();
	if(!obj || !data_cali)
...
static int KXTJ2_1009_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		if(gs_cali.valid == 1){
			acc[KXTJ2_1009_AXIS_X] -= gs_cali.x;
			acc[KXTJ2_1009_AXIS_Y] -= gs_cali.y;
			acc[KXTJ2_1009_AXIS_Z] -= gs_cali.z;
		}
...
{
	struct kxtj2_1009_i2c_data *obj = i2c_get_clientdata(client);
	int acc_tmp[KXTJ2_1009_AXES_NUM]={0};
	if(NULL == client)
	{
		return -EINVAL;
	}
	acc_tmp[obj->cvt.map[KXTJ2_1009_AXIS_X]] = obj->cvt.sign[KXTJ2_1009_AXIS_X]* data[KXTJ2_1009_AXIS_X];
	acc_tmp[obj->cvt.map[KXTJ2_1009_AXIS_Y]] = obj->cvt.sign[KXTJ2_1009_AXIS_Y]* data[KXTJ2_1009_AXIS_Y];
	acc_tmp[obj->cvt.map[KXTJ2_1009_AXIS_Z]] = obj->cvt.sign[KXTJ2_1009_AXIS_Z]* data[KXTJ2_1009_AXIS_Z];
	data[KXTJ2_1009_AXIS_X] = acc_tmp[KXTJ2_1009_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[KXTJ2_1009_AXIS_Y] = acc_tmp[KXTJ2_1009_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[KXTJ2_1009_AXIS_Z] = acc_tmp[KXTJ2_1009_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	return 0;
}
...
static long kxtj2_1009_unlocked_ioctl(struct file *file, unsigned int cmd,unsigned long arg)
{
	...
	long err = 0;
	int cali[3];
	s16	raw_data[KXTJ2_1009_AXES_NUM+1] = {0};
	int acc_tmp[KXTJ2_1009_AXES_NUM] = {0};
	...
	switch(cmd)
	{
		...
		case GSENSOR_SET_ACC_MODE: 
			...
			if(enable == 1)
				err = KXTJ2_1009_SetPowerMode(client,true);
			else
		...
		case GSENSOR_IOCTL_READ_RAW_DATA:
			...
			KXTJ2_1009_SetPowerMode(obj->client, true);
			if(err = KXTJ2_1009_ReadData(client, raw_data))
			{		 
				GSE_ERR("I2C error: ret");
				return -3;
			}
			else
			{
				...
				err = kxtj2_1009_remap_to_gravity(client, acc_tmp);
				if(err){
					err = -EFAULT;
					break;
				}
		...
		case GSENSOR_IOCTL_SET_CALI_NEW:
			...
			err = KXTJ2_1009_WriteCalibration_New(client, &gs_cali_temp);
			if(err){
				err = -EFAULT;
				break;
			}
		...
		unsigned long arg)
 {
	 ...
	 switch (cmd)
	 {
		 ...
		 break;
		 case COMPAT_GSENSOR_SET_ACC_MODE:
			 if (arg32 == NULL)
			 {
				 err = -EINVAL;
				 break;
			 }
			 err = file->f_op->unlocked_ioctl(file, GSENSOR_SET_ACC_MODE, arg32);
			 if (err){
				 GSE_ERR("GSENSOR_SET_ACC_MODE unlocked_ioctl failed.");
				 return err;
			 }
		 break;
		 case COMPAT_GSENSOR_IOCTL_SET_CALI_NEW:
			 ...
			 err = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_SET_CALI_NEW, arg32);
			 if (err){
				 GSE_ERR("GSENSOR_IOCTL_SET_CALI_NEW unlocked_ioctl failed.");
				 return err;
			 }
		 break;
		 default:
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	exit_create_attr_failed:
		kxtj2_1009_delete_attr(&(kxtj2_1009_gsensor_driver.driver));
