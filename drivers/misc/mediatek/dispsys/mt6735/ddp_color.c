...
extern BOOL DISP_GetPQ_standard_param_LCD(DISP_PQ_PARAM *pq_parameter);
extern BOOL DISP_GetPQ_vivid_param_LCD(DISP_PQ_PARAM *pq_parameter) ;
#endif
...
static int _color_io(DISP_MODULE_ENUM module, int msg, unsigned long arg, void *cmdq)
{
  ...
  #ifdef CONFIG_WT_GAMMA_PQ_WITH_MULTI_LCM
		DISP_PQ_PARAM pq_standard_param_drv;
		DISP_PQ_PARAM pq_vivid_param_drv;
#endif
    ...
    switch (msg) {
		...
		case DISP_IOCTL_GET_PQSTANDARD_PARAM_FROM_LCDDRV:
			 // printk("zhengzhou  : DISP_IOCTL_GET_PQSTANDARD_PARAM_FROM_LCDDRV\n");
			 if(DISP_GetPQ_standard_param_LCD(&pq_standard_param_drv))
			 {
	             if(copy_to_user((void *)arg, &pq_standard_param_drv, sizeof(DISP_PQ_PARAM)))
	             {
	            
	                   //printk("disp driver : WT_GAMMA_TUNNING_WITH_MULTI_LCM Copy to user failed\n");
	                   return -EFAULT;            
	             }
			 }
			 else
		...
		case DISP_IOCTL_GET_PQVIVID_PARAM_FROM_LCDDRV:
			//printk("zhengzhou  : DISP_IOCTL_GET_PQVIVID_PARAM_FROM_LCDDRV\n");
			 if(DISP_GetPQ_vivid_param_LCD(&pq_vivid_param_drv))
			 {
	             if(copy_to_user((void *)arg, &pq_vivid_param_drv, sizeof(DISP_PQ_PARAM)))
	             {
	                  // printk("disp driver : DISP_IOCTL_GET_PQPARAM_FROM_LCDDRV Copy to user failed\n");
	                   return -EFAULT;            
	             }
			 }
			 else
