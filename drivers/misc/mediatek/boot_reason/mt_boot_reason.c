...
char emmc_device_name[30]; // for flash info in factory mode ; porting from kk by billy@wt 2015/4/27

#ifdef CONFIG_OF
static int __init dt_get_boot_reason(unsigned long node, const char *uname, int depth, void *data)
{
	char *ptr = NULL, *br_ptr = NULL;
	char *emmc_ptr = NULL, *emmc_name_ptr = NULL, *q; // for flash info ; porting from kk by billy@wt 2015/4/27
	...
	// for flash info in factory mode; porting from kk by billy@wt 2015/4/27
	emmc_ptr = (char *)of_get_flat_dt_prop(node, "bootargs", NULL);
	if (emmc_ptr) 
	{
		if ((emmc_name_ptr = strstr(ptr, "emmc_name=")) != 0)
		{
			emmc_name_ptr += 10;
			if((emmc_name_ptr - emmc_ptr) > strlen(emmc_ptr+1))
			{
				printk("[wt]error: can't find emmc name in cmdline");
			}
			q = emmc_name_ptr;
			while(*q != ' ' && *q != '\0') q++;

			memset((void*)emmc_device_name, 0, sizeof(emmc_device_name));
			strncpy((char*)emmc_device_name, (const char*)emmc_name_ptr, (int)(q-emmc_name_ptr));
			printk("\r\n [wt] cmdline emmc_flash info =  %s \r\n",emmc_device_name);
        }
		else
		{
            printk("[wt]cannot find emmc name in cmdline");
		}
		pr_debug("%s\n", emmc_ptr);
	} else
	{
