 ...
 * configuration
*******************************************************************************/
#define LUX_PER_COUNT		500            // 1100 = 1.1 * 1000
#define PS_DRIVE				EPL_DRIVE_120MA
#define POLLING_MODE_HS		0

#define PS_GES 0
#define ELAN_WRITE_CALI  0
...
static int elan_epl2182_psensor_enable(struct epl2182_priv *epl_data, int enable)
{
    ...
    if(enable)
    {
        ...
        //wt add for ps calibration start
        int databuf[2];   
        databuf[0] = atomic_read(&epl_data->ps_thd_val_low);
        databuf[1] = atomic_read(&epl_data->ps_thd_val_high);
        set_psensor_intr_threshold(databuf[0],databuf[1]);
        //wt add for ps calibration end
        ...
        if(epl_data->hw->polling_mode_ps == 0)
        {
            if(test_bit(CMC_BIT_ALS, &epl_data->enable))
            {
                ...
                if((ps_state_tmp==0 && gRawData.ps_raw > databuf[1]) ||
                    (ps_state_tmp==1 && gRawData.ps_raw < databuf[0]))
                {
...
long epl2182_read_ps(struct i2c_client *client, u16 *data)
{
    ...
    {
        APS_LOG("[%s]: ALS+PS mode \r\n", __func__);
        if((new_ps_state==0 && gRawData.ps_raw > databuf[1]) ||
            (new_ps_state==1 && gRawData.ps_raw < databuf[0]))
        {
...
static void epl2182_eint_work(struct work_struct *work)
{
    ...
    int databuf[2];   
    databuf[0] = atomic_read(&epld->ps_thd_val_low);
    databuf[1] = atomic_read(&epld->ps_thd_val_high);
    ...
    else if(test_bit(CMC_BIT_PS, &epld->enable))
    {
        ...
        if(epld->pending_intr)
        {
            ...
            {
                APS_LOG("ALS+PS mode \r\n");
                if((ps_state==0 && gRawData.ps_raw > databuf[1]) ||
                    (ps_state==1 && gRawData.ps_raw < databuf[0]))
                {
...
static ssize_t epl2182_show_status(struct device_driver *ddri, char *buf)
{
    ...
    len += snprintf(buf+len, PAGE_SIZE-len, "default threshold is %d/%d\n",epld->hw->ps_threshold_low, epld->hw->ps_threshold_high);
    len += snprintf(buf+len, PAGE_SIZE-len, "cali value close : %d, far_away: %d, valid : %d\n", ps_cali.close, ps_cali.far_away, ps_cali.valid);
...
//wt add for ps calibration start
static int epl2182_read_data_for_cali(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali)
{
#if 1 //modify by Eminent Robert at 2015/05/21
	struct epl2182_priv *obj = i2c_get_clientdata(client);
	bool enable_ps = test_bit(CMC_BIT_PS, &obj->enable) && atomic_read(&obj->ps_suspend)==0;
	bool enable_als = test_bit(CMC_BIT_ALS, &obj->enable) && atomic_read(&obj->als_suspend)==0;
	...
	for(i = 0;i<20;i++)
	{
		mdelay(ALS_DELAY+2*PS_DELAY+30);//mdelay(5);fangliang
		if(DYN_ENABLE || (obj->hw->polling_mode_ps == 0))
		{
			sum += gRawData.ps_raw;
			APS_LOG("[csq++]data[%d] = %d\n", i, gRawData.ps_raw);
		}
		else
		{
			epl2182_read_ps(client,&data[i]);
			sum += data[i];
			APS_LOG("[csq++]data[%d] = %d\n", i, data[i]);
		}
	}
	...
	{
		data_cali = sum/20;
		APS_LOG("[csq++]epl2182_read_data_for_cali data_cali = %d\n",data_cali);
		ps_data_cali->far_away = data_cali + CUST_EPL_PS_THRES_FAR;
		ps_data_cali->close = data_cali + CUST_EPL_PS_THRES_CLOSE;
	...
	else
	{
            	...
            	APS_LOG("epl2182_read_data_for_cali get data error!\n");
            	err=  -1;
	}
	 ...
	 for(i = 0;i<20;i++)
	 	{
			...
			if(!epl2182_read_ps(client,&data[i]))
			{
				sum += data[i];
			}
			else
...
static void epl2182_WriteCalibration(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *data_cali)
{
	struct epl2182_priv *obj = i2c_get_clientdata(client);
...
static long epl2182_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case ALSPS_SET_PS_CALI:
            ...
            epl2182_WriteCalibration(obj->client,&ps_cali_temp);
        ...
        case ALSPS_GET_PS_RAW_DATA_FOR_CALI:
            ...
            epl2182_read_data_for_cali(obj->client,&ps_cali_temp);
...
static int epl2182_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    hardwareinfo_set_prop(HARDWARE_ALSPS, "epl2182");
      }
