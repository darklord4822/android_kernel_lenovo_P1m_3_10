...
static int __init devinfo_parse_dt(unsigned long node, const char *uname, int depth, void *data)
{
    ...
    struct tag *tag_revision;
    tag_revision = of_get_flat_dt_prop(node, "atag,revision", NULL);
    if (tag_revision) UbVbat = tag_revision->u.revision.Vbat;
    pr_info("Load default dfo data...UbVbat=%d\n",UbVbat);
